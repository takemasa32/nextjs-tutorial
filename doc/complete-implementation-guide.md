...
v1.0（従来型）から v5.0（最新型）まで、5 つの段階を通じて実装の進化を体験できます。
...
- クライアントサイドで `fetch` を使用して API を呼び出し
...
- `route.test.ts` を削除（API Route が不要になるため）
- `_actions.test.ts` を拡張
- E2E テストは変更なし（ユーザー体験は同じため）
...
| v2.0-action-created           | 30 分    | Server Actions 作成    |
| v3.0-form-refactored          | 20 分    | フォーム接続変更       |
| v4.0-useformstatus-introduced | 25 分    | モダンローディング     |
| v5.0-tests-updated            | 35 分    | テスト更新と最終確認   |
